...
class GraphSAGE(nn.Module):
    r"""GraphSAGE (Hamilton et al., 2017).

    Implements a variable-depth GraphSAGE using PyG's
    :class:`~torch_geometric.nn.SAGEConv`.
    Architecture: [SAGEConv + ReLU + Dropout] × (depth-1) → SAGEConv.

    GraphSAGE uses a separate learnable matrix for the ego-embedding,
    which distinguishes it from GCN. PyG's SAGEConv handles this internally.
    ...
    Args:
        ...
        dropout (float): Dropout probability applied after each hidden layer.
            Default: ``0.5``.
        depth (int): Total number of SAGE layers (must be ≥ 1). Default: ``2``.
    """
